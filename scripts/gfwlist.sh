...
function gen_gfwlist_pac() {
  ...
  sed -i 's/^/    "/' gfwlist_domains.tmp
  sed -i 's/$/": 1,/' gfwlist_domains.tmp
  sed -i '$ s/": 1,/": 1/g' gfwlist_domains.tmp

  # replace content
  sed -i 's/___GFWLIST_DOMAINS_PLACEHOLDER___/cat gfwlist_domains.tmp/e' gfwlist.pac
...
function dist_release() {
  mkdir -p $DIST_DIR
  mv $TMP_DIR/gfwlist.pac $DIST_DIR
}
