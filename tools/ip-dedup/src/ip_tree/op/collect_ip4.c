...
static int _ip4_tree_collect_addr_into (
...
int ip4_tree_collect_addr_into (
   ...
   if ( (tree == NULL) || (darr == NULL) ) {
      ...
      return _ip4_tree_collect_addr_into (
...
struct dynarray* ip4_tree_collect_addr ( struct ip_tree* const restrict tree ) {
   ...
   if ( darr != NULL ) {
      ...
      if ( ip4_tree_collect_addr_into ( tree, darr ) != 0 ) {
         dynarray_free ( darr );
         darr = NULL;
      }
   }
...
static int _ip4_tree_collect_addr_into (
   ...
   } else if ( _ip4_tree_collect_addr_into ( tdesc, darr, node->left ) != 0 ) {
      return -1;

   } else if ( _ip4_tree_collect_addr_into ( tdesc, darr, node->right ) != 0 ) {
      return -1;
